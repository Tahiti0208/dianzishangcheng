        ...
        <div class="mag-box" v-for="(item,index) in orderList" :key="index">
            ...
            <el-row :gutter="20">
                ...
                <el-col :offset="4" :span="4">
                    <el-button v-if="item.orderType == 0" @click="continupay(item.id,item.totalPrice)" class="pay">立即支付</el-button>
    ...
    export default {
        ...
        methods:{
            ...
            open(id,price){
                ...
                this.$msgbox({
                    ...
                    center:true,
                    message: h('p', null, [
                        h('p', {style:'color:#ff6700;font-size:18px;text-align:center;'}, price +'元'),
                    ]),
        ...
        created() {
            this.orderList = JSON.parse(
                JSON.stringify(this.$store.state.order.orderList)
            )
        }
    }
</script>
